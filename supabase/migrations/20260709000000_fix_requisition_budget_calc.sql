-- Migration: Fix create_requisition_secure to only count approved_amount for Approved requisitions
...
BEGIN
    ...
    -- 4. Calculate total committed amount so far (excluding Cancelled requisitions, counting approved_amount for Approved requisitions)
    SELECT COALESCE(
        SUM(
            CASE 
                WHEN requisition_status = 'Approved'::public.requisition_status_enum 
                THEN COALESCE(approved_amount, requisition_amount)
                ELSE requisition_amount
            END
        ), 0
    ) INTO v_committed
